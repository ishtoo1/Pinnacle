...
public class BatchDaoImpl implements BatchDao {
	...
	public void addBatch(Batch batch) {
		...
		template.update(sql, batch.getBatchId(), batch.getBatchName(), batch.getStartingDate(), batch.getEndDate(), true);
	}
	...
	public Batch findBatchById(String batchId) {
		...
		try {
			return (Batch)template.queryForObject(sql, new Object[] {batchId}, new BeanPropertyRowMapper<>(Batch.class));
		}
		catch (Exception e) {
			return null;
		}
	...
	public List<Batch> findAllBatchesHavingTeacher(int teacherId) {
		String sql="select distinct batch.batchId, batchName, startingDate, endDate, isOpen from "
				+ "(select * from Subject where teacherId=?) as A "
