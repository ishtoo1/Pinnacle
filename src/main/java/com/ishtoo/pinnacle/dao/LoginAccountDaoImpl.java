...
public class LoginAccountDaoImpl implements LoginAccountDao {
	...
	public void addLoginAccount(LoginAccount loginAccount) {
		...
		template.update(sql, loginAccount.getUsername(), loginAccount.getPassword(), loginAccount.getRole(),
				loginAccount.getEnabled());
	}
	...
	public boolean checkLoginCredentials(LoginAccount loginAccount) {
		...
		return template.queryForObject(sql, Boolean.class,
				new Object[] { loginAccount.getUsername(), loginAccount.getPassword() });
	}

	@Override
	public LoginAccount findByUsername(String username) {
		String sql = "select * from LoginAccount where username=?";
		return (LoginAccount) template.queryForObject(sql, new Object[] { username },
